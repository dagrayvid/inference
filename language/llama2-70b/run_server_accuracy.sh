...
python3 -u main.py --scenario Server --model-path ${CHECKPOINT_PATH} --api-server ${API_HOST} --additional-servers ${ADDITIONAL_SERVERS} --api-model-name Llama-2-70b-chat-hf --mlperf-conf mlperf.conf --vllm --user-conf user.conf --total-sample-count 24576 --dataset-path ${DATASET_PATH} --output-log-dir ${LOGDIR} --dtype float32 --device cpu --accuracy

ACCURACY_LOG_FILE=${LOGDIR}/mlperf_log_accuracy.json
